...
connection = pymysql.connect(host='localhost', user='root', password='Vij@y385717', database='sra')
...
def insert_data(name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills,
                courses):
    alter_column_sql = "ALTER TABLE user_data MODIFY COLUMN Actual_skills VARCHAR(500)"
...
def run():
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'vijay123' and ad_password == 'vijay123':
                st.success("Welcome VijayKumar")
                # Display Data
                ...
                fig = px.pie(names=labels, values=values, title="User's Experienced Level")
                st.plotly_chart(fig)
